      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const response = await invokeLLM({
          messages: [
            ...
            { role: "user", content: `Hozz létre egy ${input.platform} bejegyzést a következő témában: "${input.topic}" a "${input.pillar}" tartalmi pillérhez.\nCélközönség: ${input.targetAudience ?? "üzleti szakemberek"}.\nFormátum: ${input.format ?? "standard bejegyzés"}.\nCTA: ${input.cta ?? "interakció a tartalommal"}.\nPlatform irányelvek: ${platformGuide[input.platform]}.\n\nAdj vissza JSON-t (MINDEN szöveg magyarul): caption (bejegyzés szövege magyarul), hashtags (hashtagek tömbje), visualBrief (vizuális brief leírása magyarul), ctaText (cselekvésre szólítás magyarul), rationale (1-2 mondat magyarul, LAIKUSNAK érthetően: MIÉRT így írtad meg — milyen szempontra/jelre építve, pl. a márka hangneme, a platform stílusa, a célközönség; NE technikai, hanem emberi indoklás)` },
          ],
          response_format: { type: "json_schema", json_schema: { name: "social_post", strict: true, schema: { type: "object", properties: { caption: { type: "string" }, hashtags: { type: "array", items: { type: "string" } }, visualBrief: { type: "string" }, ctaText: { type: "string" }, rationale: { type: "string" } }, required: ["caption", "hashtags", "visualBrief", "ctaText", "rationale"], additionalProperties: false } } },
        });
        const content = response.choices[0]?.message?.content ?? "{}";
        await recordAiUsage(ctx.appUser.id, "post", ctx.appUser.role);
        return parseLLMJson(content);
