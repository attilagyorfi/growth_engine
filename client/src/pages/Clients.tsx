...
const PLAN_LABELS: Record<string, { label: string; color: string }> = {
  free:    { label: "Free",    color: "oklch(0.55 0.015 240)" },
  starter: { label: "Starter", color: "oklch(0.65 0.18 165)" },
  pro:     { label: "Pro",     color: "oklch(0.6 0.2 255)" },
  agency:  { label: "Agency",  color: "oklch(0.75 0.18 75)" },
};
    ...
    return (
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center h-64 gap-4">
          <XCircle size={40} style={{ color: "oklch(0.65 0.22 25)" }} />
          <p className="text-sm font-semibold" style={{ color: "oklch(0.65 0.015 240)" }}>
  ...
  return (
    <DashboardLayout>
      ...
      <div className="relative mb-5">
        <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: "oklch(0.5 0.015 240)" }} />
        <input
          value={search}
          onChange={e => setSearch(e.target.value)}
          placeholder="Keresés név, email, cég alapján..."
          className="w-full pl-9 pr-4 py-2.5 rounded-lg text-sm border"
          style={{ background: "oklch(0.17 0.022 255)", borderColor: border, color: "oklch(0.88 0.008 240)" }}
        />
      </div>
        ...
        {PLAN_OPTIONS.map(plan => {
          ...
          return (
            <div key={plan} className="rounded-xl border p-4" style={{ background: cardBg, borderColor: border }}>
              ...
              <p className="text-2xl font-bold" style={{ color: "oklch(0.92 0.008 240)", fontFamily: "Sora, sans-serif" }}>{count}</p>
            </div>
          );
        })}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-40 gap-3">
          <Users size={32} style={{ color: "oklch(0.4 0.015 240)" }} />
          <p className="text-sm" style={{ color: "oklch(0.5 0.015 240)" }}>Nincs találat</p>
        </div>
      ) : (
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <p className="text-sm font-bold" style={{ color: "oklch(0.92 0.008 240)", fontFamily: "Sora, sans-serif" }}>
                      ...
                      {client.active ? (
                        <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ background: "oklch(0.65 0.18 165 / 15%)", color: "oklch(0.65 0.18 165)" }}>
                          Aktív
                        </span>
                      ) : (
                        <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ background: "oklch(0.65 0.22 25 / 15%)", color: "oklch(0.65 0.22 25)" }}>
                          Inaktív
                        </span>
                      )}
                      {client.onboardingCompleted ? (
                        <span className="text-xs flex items-center gap-1" style={{ color: "oklch(0.65 0.18 165)" }}>
                          <CheckCircle size={11} /> Onboarding kész
                        </span>
                      ) : (
                        <span className="text-xs flex items-center gap-1" style={{ color: "oklch(0.75 0.18 75)" }}>
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1">
                      <span className="text-xs flex items-center gap-1" style={{ color: "oklch(0.55 0.015 240)" }}>
                      ...
                      <span className="text-xs" style={{ color: "oklch(0.45 0.015 240)" }}>
                    ...
                    {client.notes && (
                      <div className="mt-2 flex items-start gap-1.5">
                        <StickyNote size={11} style={{ color: "oklch(0.75 0.18 75)", flexShrink: 0, marginTop: 2 }} />
                        <p className="text-xs line-clamp-2" style={{ color: "oklch(0.6 0.015 240)" }}>{client.notes}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <button onClick={() => openEdit(client)}
                    ...
                    <Pencil size={13} style={{ color: "oklch(0.6 0.015 240)" }} />
                  </button>
                </div>
              </div>
            );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "oklch(0 0 0 / 70%)" }}>
          <div className="w-full max-w-md rounded-2xl border p-6" style={{ background: "oklch(0.15 0.022 255)", borderColor: "oklch(1 0 0 / 12%)" }}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => { setEditModal(false); setSelectedClient(null); }} style={{ color: "oklch(0.5 0.015 240)" }}>
                <X size={18} />
              </button>
            </div>

            <div className="mb-3 p-3 rounded-lg" style={{ background: "oklch(0.22 0.02 255)" }}>
              <p className="text-xs font-semibold" style={{ color: "oklch(0.92 0.008 240)" }}>{selectedClient.name ?? "–"}</p>
              <p className="text-xs mt-0.5" style={{ color: "oklch(0.55 0.015 240)" }}>{selectedClient.email}</p>
            </div>

            <div className="space-y-3">
              {/* Company name */}
              <div>
                <label className="text-xs font-semibold mb-1 block" style={{ color: "oklch(0.65 0.015 240)" }}>Cég neve</label>
              ...
              {/* Contact person */}
              <div>
                <label className="text-xs font-semibold mb-1 block" style={{ color: "oklch(0.65 0.015 240)" }}>Kapcsolattartó személy</label>
                <input value={editForm.contactPerson ?? ""} onChange={e => setEditForm((f: any) => ({ ...f, contactPerson: e.target.value }))}
                  className="w-full px-3 py-2 rounded-lg text-sm border" style={{ background: "oklch(0.22 0.02 255)", borderColor: "oklch(1 0 0 / 10%)", color: "oklch(0.88 0.008 240)" }} />
              </div>
              {/* Phone */}
              <div>
                <label className="text-xs font-semibold mb-1 block" style={{ color: "oklch(0.65 0.015 240)" }}>Telefonszám</label>
                <input value={editForm.phone ?? ""} onChange={e => setEditForm((f: any) => ({ ...f, phone: e.target.value }))}
                  className="w-full px-3 py-2 rounded-lg text-sm border" style={{ background: "oklch(0.22 0.02 255)", borderColor: "oklch(1 0 0 / 10%)", color: "oklch(0.88 0.008 240)" }} />
              </div>
              {/* Subscription plan */}
              <div>
                <label className="text-xs font-semibold mb-1 block" style={{ color: "oklch(0.65 0.015 240)" }}>Előfizetési csomag</label>
              ...
              <div className="flex items-center gap-3">
                <label className="text-xs font-semibold" style={{ color: "oklch(0.65 0.015 240)" }}>Aktív ügyfél</label>
                <button onClick={() => setEditForm((f: any) => ({ ...f, active: !f.active }))}
                  className="relative w-10 h-5 rounded-full transition-colors flex-shrink-0"
                  style={{ background: editForm.active ? "oklch(0.65 0.18 165)" : "oklch(0.3 0.02 255)" }}>
              ...
              {/* Notes */}
              <div>
                <label className="text-xs font-semibold mb-1 block" style={{ color: "oklch(0.65 0.015 240)" }}>Megjegyzések (belső)</label>
